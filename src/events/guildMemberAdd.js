			...
			if (inviterId) {
				...
				if (channelId) {
					...
					if (channel && channel.isTextBased()) {
						...
						const message = `🎉 ${member} joined — invited by <@${inviterId}> using code \`${inviteCode}\` (uses: ${inviteUsesNow}). Invites: ${total} (Reg ${inviterStats.regularInvites} • Fake ${inviterStats.fakeInvites} • Left ${inviterStats.leftInvites} • PF ${inviterStats.paidReferrals} • FO ${inviterStats.freeOrders} • $5 Orders ${inviterStats.noFeeOrders}).`;
